...
import { deletePatientInteraction } from "@/lib/private/interactionHandler";
...
export default function DeleteInteraction({ values, onClose, reload} : Props) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        try {
            ...
            const res = await deletePatientInteraction(id)
    ...
    return(
        <form onSubmit={handleSubmit} className="relative flex flex-col gap-6 p-4">
            ...
            <section className="flex flex-col gap-4">
                <p>¿Deseas eliminar esta interacción?</p>
            </section>
