...
export default function DoctorSideAppointmentListCard({ appointment, handleModifyAppointment, handleDeleteAppointment }: Props) {
    ...
    return(
        <li className="flex gap-2 items-center">
            <div className="flex gap-4 text-cool-gray-700 items-center border border-cool-gray-100 rounded-lg *:p-2 w-full">
                <div className="w-16">
                    <Avatar url={appointment.patient?.user ? (routes.api.base + appointment.patient?.user?.avatar?.fileUrl) : ""} className="size-8" />
                </div>
            ...
            {appointment.status != AppointmentStatus.canceled && <div className="w-12 flex gap-2">
